...
import {
  ...
  Scale, DollarSign,
  Bot, Users, Radio, Database, Eye, ShieldCheck, Lock, Play
} from 'lucide-react';
...
const verticalIcons = [Users, Bot, Radio, Database];

const trustBarIcons = [Eye, ShieldCheck, Scale, Lock];

// Click-to-play video card — iframe injected only on user click
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-5 sm:px-6 text-center pt-20 pb-10">
          ...
          <div className="hero-cta flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
            <ContactCTA
              label={t.ui.requestDemo || t.hero.cta.primary.text}
              className="btn-primary w-full sm:w-auto justify-center"
              testId="hero-cta-primary"
            />
            <Link href="/technology" data-testid="hero-cta-secondary" className="btn-secondary-dark w-full sm:w-auto justify-center">
...
VERDICT         \u2588\u2588 GATE HELD \u2014 OUTPUT NEVER REACHED WORKFLOW`}</pre>
          ...
          <div className="reveal text-center mt-8 sm:mt-10 space-y-3">
            ...
            <div className="mt-6">
              <Link href="/dashboard" className="btn-secondary">{t.evidencePack.ctaLabel}</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-spacing bg-[#111111] relative overflow-hidden">
        <div className="max-w-5xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="reveal text-center">
            <Link href="/technology" className="inline-flex items-center gap-2 text-sm text-[#E8761D] hover:text-[#F5993D] font-medium transition-colors duration-200">
      ...
      {/* ===== DIGITAL OMNIBUS URGENCY BANNER ===== */}
      <div className="bg-[#1A0A00] border-y border-[#E8761D]/40 py-5 px-5 sm:px-6">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="flex-shrink-0">
            <span className="inline-flex items-center gap-1.5 bg-[#E8761D] text-black text-[10px] font-black uppercase tracking-[0.2em] px-2.5 py-1 rounded-sm">
              <AlertTriangle size={11} />
              August 2, 2026
            </span>
          </div>
          <p className="text-white/90 text-sm leading-relaxed">
            <strong className="text-[#E8761D] font-bold">The EU Digital Omnibus delayed the noise, but locked in the trap.</strong>{" "}
            Article 50 Transparency enforcement hits August 2, 2026. CISOs relying on probabilistic guardrails will fail audits.{" "}
            <span className="text-white font-semibold">Deploy deterministic runtime enforcement today.</span>
          </p>
        </div>
      </div>
      ...
      <section className="section-spacing bg-[#F7F7F7]">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {t.team.map((member, i) => (
              <div key={i} className={`reveal reveal-delay-${i+1} pro-card p-6 sm:p-8 group relative overflow-hidden`}>
                ...
                <p className="text-sm text-[#555555] leading-relaxed mb-4">{member.bio}</p>
                {member.hoverLine && (
                  <p className="text-xs text-[#E8761D]/80 italic leading-relaxed border-t border-[#E5E5E5] pt-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    "{member.hoverLine}"
                  </p>
                )}
                {member.linkedin && (
                  <a
                    ...
                    itemProp="sameAs"
                    className="inline-flex items-center gap-1.5 text-xs text-[#E8761D] hover:text-[#F5993D] font-semibold tracking-wide uppercase transition-colors duration-200 border-t border-[#E5E5E5] pt-3 w-full"
                  >
        ...
        <div className="max-w-3xl mx-auto px-5 sm:px-6 text-center">
          <div className="reveal">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
              <ContactCTA
                label={t.ui.requestDemo || t.ui.requestPitchDeck}
                className="btn-primary w-full sm:w-auto justify-center"
              />
              <Link href="/technology" className="btn-secondary w-full sm:w-auto justify-center">
