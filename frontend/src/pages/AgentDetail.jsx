...
export default function AgentDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const d = await fetchAgent(entityId, 100);
        if (alive) setData(d);
      } catch (e) {
        if (alive)
          setError(
      ...
      >
              ...
              {data && (
                <div className="flex items-center gap-3 mt-2 flex-wrap text-[11px] text-zinc-500">
                  ...
                  <span className="text-mono">
                    first seen {formatTime(data.first_seen)}
                  </span>
                </div>
              )}
            </div>
        ...
        {loading ? (
          ...
          <div
            ...
            className="glass-panel rounded-2xl p-8 text-center text-zinc-400"
          >
            {error}
          </div>
        ) : (
          <>
            <section
              data-testid="agent-kpis"
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 fade-up"
            >
              <KpiCard
                testId="agent-kpi-risk"
                label="Agent Risk Score"
                value={`${data.risk_score}/100`}
                subValue="KMIR composite"
                icon={Gauge}
                accent={data.risk_score >= 70 ? "red" : "emerald"}
              />
              <KpiCard
                testId="agent-kpi-blocked"
                label="Blocked Actions"
                value={data.blocked.toLocaleString()}
                subValue="DLP interceptions"
                icon={ShieldAlert}
                accent="red"
              />
              <KpiCard
                testId="agent-kpi-alerts"
                label="Compliance Alerts"
                value={data.alerts.toLocaleString()}
                subValue="policy warnings"
                icon={AlertOctagon}
                accent="violet"
              />
              <KpiCard
                testId="agent-kpi-allowed"
                label="Allowed Actions"
                value={data.allowed.toLocaleString()}
                subValue={`of ${data.total_events} total`}
                icon={CheckCircle2}
                accent="emerald"
              />
            </section>
            ...
            <section className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 fade-up">
              <div
                data-testid="agent-top-rules"
                className="glass-panel rounded-2xl p-5 sm:p-6"
              >
                <h3 className="text-sm font-semibold text-zinc-100 tracking-tight mb-1">
                  Top Violated Rules
                </h3>
                <p className="text-[11px] text-zinc-500 tracking-wide mb-4">
                  Across BLOCKED + ALERT events
                </p>
                  ...
                  {data.top_rules.map((r, i) => (
                    <div key={r.rule} data-testid={`rule-row-${i}`}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="text-mono text-[11px] text-orange-300 truncate">
                          {r.rule}
                        </div>
                  ...
                  <table className="w-full min-w-[640px]">
                    <thead className="sticky top-0 bg-zinc-950/80 backdrop-blur-xl">
                      <tr className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 border-b border-white/5">
                        <th className="text-left font-medium px-5 sm:px-6 py-3 w-[120px]">
                          Timestamp
                        </th>
                        <th className="text-left font-medium px-3 py-3">
                          Action Intent
                        </th>
                        <th className="text-left font-medium px-3 py-3 w-[200px]">
                          KMIR Rule
                        </th>
                        <th className="text-left font-medium px-3 py-3 w-[110px]">
                          Status
                        </th>
                        <th className="text-right font-medium px-5 sm:px-6 py-3 w-[70px]">
                          Details
                        </th>
                      </tr>
                    </thead>
                    <tbody className="text-mono">
                      {data.timeline.map((e) => (
                        <tr
                          key={e.id}
                          data-testid={`timeline-row-${e.id}`}
                          onClick={() => handleSelect(e)}
                          className="cursor-pointer border-b border-white/5 hover:bg-white/[0.025] transition-colors"
                        >
                          <td className="px-5 sm:px-6 py-3 text-[11px] text-zinc-500 whitespace-nowrap">
                            {formatTime(e.timestamp)}
                          </td>
                          <td className="px-3 py-3 text-[11px] text-zinc-300 max-w-[280px] truncate">
                            {e.action_intent}
                          </td>
